...
# ALB (moet 2 public subnets hebben)
resource "aws_lb" "app" {
  name               = "case1nca-alb"
...
# ECS
resource "aws_ecs_cluster" "this" { name = "case1-nca" }
...
resource "aws_cloudwatch_log_group" "ecs" { name = "/ecs/app" retention_in_days = 7 }
...
resource "aws_ecs_task_definition" "app" {
  ...
  container_definitions = jsonencode([{
    ...
    secrets = [{ name = "DB_PASS", valueFrom = aws_secretsmanager_secret.db_pass.arn }]
    logConfiguration = {
...
resource "aws_ecs_service" "app" {
  ...
  #public subnets + public IP (ALB blijft de enige ingang)
